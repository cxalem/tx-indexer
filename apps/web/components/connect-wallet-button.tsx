  ...
  return (
    <div className="relative">
      <Button
        ...
        className="cursor-pointer lowercase bg-vibrant-red text-white shadow-md hover:bg-vibrant-red/80 transition-colors duration-300 min-w-[160px]"
      >
        {address ? (
          <span className="font-mono max-w-[12ch] truncate">
            {truncate(address)}
          </span>
        ) : (
      ...
      {open ? (
        <div className="absolute right-0 z-10 mt-2 w-full min-w-[240px] rounded-lg border border-gray-200 bg-white shadow-lg">
          {isConnected ? (
            ...
            <div className=" pb-2">
              <p className="text-xs lowercase font-medium text-neutral-500 px-3 py-2">
                Choose Wallet
              </p>
              <div className="space-y-1.5">
                {CONNECTORS.map((connector) => (
                  <Button
                    ...
                    variant="outline"
                    className="cursor-pointer shadow-none border-0 w-full justify-between lowercase hover:bg-neutral-50"
                  >
